...
<template>
  ...
  >
    ...
    />
    <div v-if="!app.IsDarkMode" absolute bottom-12px w-full flex justify-center gap-2.5>
      <div
      ...
      />
      <div
        v-if="collSubMenu" :class="`i-carbon:side-panel-close ${mainMenuInverted ? 'text-white' : ''}`" rotate-180
        cursor-pointer text-18px @click="collSubMenu = !collSubMenu"
      />
      <div
        v-else :class="`i-carbon:side-panel-close ${mainMenuInverted ? 'text-white' : ''}`" cursor-pointer text-18px
